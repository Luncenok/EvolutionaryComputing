// AMSEA Islands FULL - 2 Islands, 4 Operators (including LNS), Adaptive Selection, Elite Archive
// Replicates all features of amsea.cpp inside an Island Model
...
std::vector<int> repairSolutionAMSEA_Full(const std::vector<int> &partial,
...
// ============================================================================
// Fast Local Search (Steepest Descent from amsea.cpp)
// ============================================================================
...
// ============================================================================
// Operator 1: Common Nodes and Edges
// ============================================================================
...
static std::vector<int> opParentRepair(const std::vector<int> &p1, const std::vector<int> &p2,
    ...
    return repairSolutionAMSEA_Full(partial, dist, costs, n, sc, 1.0, 1.0);
}
...
static std::vector<int> opPathRelinkEnhanced(
    ...
    for (double ratio : {0.25, 0.5, 0.75}) {
        ...
        auto repaired = repairSolutionAMSEA_Full(partial, dist, costs, n, sc, 1.0, 1.0);
...
// ============================================================================
// Perturbation - STRONG VERSION (5-8 2-opt moves + 50% Node Swap)
// ============================================================================
...
// ============================================================================
// AMSEA Islands FULL - Main Function
// ============================================================================
...
AMSEAIslandsFullResult amseaIslandsFull(int n, int selectCount,
    ...
    AMSEAIslandsFullResult result;
    result.bestObjective = INT_MAX;
    result.generations = 0;
    for (int i = 0; i < 4; i++) { result.operatorSuccesses[i] = 0; result.operatorAttempts[i] = 1; }
    
    auto startTime = std::chrono::high_resolution_clock::now();
    
    const int NUM_ISLANDS = 2;
    const int ISLAND_SIZE = populationSize / NUM_ISLANDS;
    ...
    for (int island = 0; island < NUM_ISLANDS; island++) {
        islands[island] = initPopulation(n, selectCount, distance, costs, rng, ISLAND_SIZE, islandObjs[island], islandSeen[island]);
        for (int i = 0; i < islands[island].size(); i++) {
            int obj = islandObjs[island][i];
            if (obj < result.bestObjective) {
                result.bestObjective = obj;
                result.bestSolution = islands[island][i];
            }
            updateArchive(elite, islands[island][i], obj, ARCHIVE_SIZE);
        }
    }
    
    // Per-island Operator Stats (Adaptive Selection)
    int islandOpSuccess[NUM_ISLANDS][4];
    int islandOpAttempts[NUM_ISLANDS][4];
    for (int i = 0; i < NUM_ISLANDS; i++) {
        for (int j = 0; j < 4; j++) {
            islandOpSuccess[i][j] = 0;
            islandOpAttempts[i][j] = 1;
        }
    }
    ...
    while (true) {
        ...
        for (int island = 0; island < NUM_ISLANDS; island++) {
            ...
            // Tournament selection (like amsea.cpp)
            auto [p1, p2] = tournamentSelect(popObj, rng);
            
            // Adaptive selection (per island)
            int op = selectOp(islandOpSuccess[island], islandOpAttempts[island], rng);
            islandOpAttempts[island][op]++;
            ...
            // Global best update and Stagnation
            ...
            // Stagnation Handling (Archive + Perturbation)
            if (stagnation[island] >= STAGNATION) {
                ...
                // Try Elite Recovery (inject global bests into this island)
                if (!elite.empty()) {
