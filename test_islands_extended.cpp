// More Island Configurations Test
// Testing: larger populations with islands, different migration rates
...
// Full AMSEA with Islands (configurable)
...
int main() {
    std::cout << "Extended Island Configurations Test\n";
    std::cout << "====================================\n";
    ...
    for (const char* instance : {"input/TSPA.csv", "input/TSPB.csv"}) {
        ...
        // Baseline: Pop 20
        {
            ...
            for (int run = 0; run < numRuns; run++) {
                ...
                std::mt19937 rng(rd());
                auto result = runFullAMSEA(n, selectCount, distance, costs, timeLimit, rng, 20, 30);
        ...
        // Pop 40 single vs 2x20 islands
        {
            ...
            for (int run = 0; run < numRuns; run++) {
                std::random_device rd;
                std::mt19937 rng(rd());
                auto result = runFullAMSEA(n, selectCount, distance, costs, timeLimit, rng, 40, 30);
                objectives.push_back(result.bestObjective);
                totalGens += result.generations;
            }
            int best = *std::min_element(objectives.begin(), objectives.end());
            double avg = std::accumulate(objectives.begin(), objectives.end(), 0.0) / numRuns;
            std::cout << "Pop=40 (single):\n";
            std::cout << "  Best=" << best << ", Avg=" << std::fixed << std::setprecision(1) << avg;
            std::cout << ", Gens=" << (totalGens / numRuns) << "\n\n";
        }
        
        {
            ...
            for (int run = 0; run < numRuns; run++) {
                std::random_device rd;
                std::mt19937 rng(rd());
                auto result = runFullAMSEA_Islands(n, selectCount, distance, costs, timeLimit, rng, 40, 2, 100);
                objectives.push_back(result.bestObjective);
                totalGens += result.generations;
            }
            int best = *std::min_element(objectives.begin(), objectives.end());
            double avg = std::accumulate(objectives.begin(), objectives.end(), 0.0) / numRuns;
            std::cout << "Pop=40 (2x20 islands, migrate 100):\n";
            std::cout << "  Best=" << best << ", Avg=" << std::fixed << std::setprecision(1) << avg;
            std::cout << ", Gens=" << (totalGens / numRuns) << "\n\n";
        }
        ...
        // Very rare migration
        {
            ...
            for (int run = 0; run < numRuns; run++) {
                std::random_device rd;
                std::mt19937 rng(rd());
                auto result = runFullAMSEA_Islands(n, selectCount, distance, costs, timeLimit, rng, 20, 2, 500);
                objectives.push_back(result.bestObjective);
                totalGens += result.generations;
            }
            int best = *std::min_element(objectives.begin(), objectives.end());
            double avg = std::accumulate(objectives.begin(), objectives.end(), 0.0) / numRuns;
            std::cout << "Pop=20 (2x10 islands, migrate 500):\n";
            std::cout << "  Best=" << best << ", Avg=" << std::fixed << std::setprecision(1) << avg;
            std::cout << ", Gens=" << (totalGens / numRuns) << "\n\n";
        }
    }
