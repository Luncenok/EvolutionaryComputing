// Comprehensive Island Parameter Test
// Testing many configurations to find if any islands setup beats single pop
...
Result runFullAMSEA(int n, int selectCount,
    ...
    for (int i = 0; i < popSize && pop.size() < popSize; i++)
...
Result runIslandAMSEA(int n, int selectCount,
    const std::vector<std::vector<int>> &distance,
    const std::vector<int> &costs, double timeLimit, std::mt19937 &rng,
    int totalPop, int numIslands, int migrationInterval, bool crossIslandCrossover) {
    ...
    const int ISLAND_SIZE = totalPop / numIslands;
    const int STAGNATION = 30;
    ...
    while (true) {
        ...
        for (int island = 0; island < numIslands; island++) {
            ...
            // Cross-island crossover option
            int p2Island = island;
            if (crossIslandCrossover && numIslands > 1 && std::uniform_real_distribution<>(0, 1)(rng) < 0.3) {
                p2Island = std::uniform_int_distribution<>(0, numIslands - 2)(rng);
                if (p2Island >= island) p2Island++;
            }
            
            std::uniform_int_distribution<> dist2(0, islands[p2Island].size() - 1);
            int p2 = dist2(rng);
            
            std::unordered_set<int> n2(islands[p2Island][p2].begin(), islands[p2Island][p2].end());
            std::vector<int> partial;
            for (int x : pop[p1]) if (n2.count(x)) partial.push_back(x);
            auto offspring = repairStandard(partial, distance, costs, n, selectCount);
...
int main() {
    std::cout << "Comprehensive Island Parameter Test\n";
    std::cout << "====================================\n";
    ...
    for (const char* instance : {"input/TSPA.csv", "input/TSPB.csv"}) {
        ...
        auto runConfig = [&](const char* name, auto runFunc) {
            std::vector<int> objectives;
            long long totalGens = 0;
            for (int run = 0; run < numRuns; run++) {
                std::random_device rd;
                std::mt19937 rng(rd());
                auto result = runFunc(rng);
            ...
            std::cout << "  Best=" << best << ", Worst=" << worst 
                      ...
                      << ", Gens=" << (totalGens / numRuns) << "\n\n";
        };
        
        // Baseline
        runConfig("Pop=20 single", [&](std::mt19937& rng) {
            return runFullAMSEA(n, selectCount, distance, costs, timeLimit, rng, 20);
        });
        
        // Different island configs
        runConfig("2 islands (10+10), migrate 50", [&](std::mt19937& rng) {
            return runIslandAMSEA(n, selectCount, distance, costs, timeLimit, rng, 20, 2, 50, false);
        });
        
        runConfig("2 islands (10+10), migrate 200", [&](std::mt19937& rng) {
            return runIslandAMSEA(n, selectCount, distance, costs, timeLimit, rng, 20, 2, 200, false);
        });
        
        runConfig("2 islands (10+10), cross-island crossover", [&](std::mt19937& rng) {
            return runIslandAMSEA(n, selectCount, distance, costs, timeLimit, rng, 20, 2, 100, true);
        });
        
        runConfig("4 islands (5+5+5+5), migrate 50", [&](std::mt19937& rng) {
            return runIslandAMSEA(n, selectCount, distance, costs, timeLimit, rng, 20, 4, 50, false);
        });
        
        runConfig("Pop=30 single", [&](std::mt19937& rng) {
            return runFullAMSEA(n, selectCount, distance, costs, timeLimit, rng, 30);
        });
        
        runConfig("3 islands (10+10+10), migrate 100", [&](std::mt19937& rng) {
            return runIslandAMSEA(n, selectCount, distance, costs, timeLimit, rng, 30, 3, 100, false);
        });
        
        runConfig("Pop=15 single", [&](std::mt19937& rng) {
            return runFullAMSEA(n, selectCount, distance, costs, timeLimit, rng, 15);
        });
    }
