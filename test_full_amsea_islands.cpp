// Full 3-Operator AMSEA + Greedy LS + Islands (20 runs)
// Testing if islands can improve Full AMSEA (69129 baseline)
...
// Full 3-operator AMSEA + Greedy LS (Single population)
...
// Full 3-operator AMSEA + Greedy LS + Islands
Result runFullAMSEA_Islands(int n, int selectCount,
    const std::vector<std::vector<int>> &distance,
    const std::vector<int> &costs, double timeLimit, std::mt19937 &rng,
    int totalPop, int numIslands, int migrationInterval) {
...
int main() {
    std::cout << "Full 3-Operator AMSEA + Greedy LS: Single vs Islands\n";
    std::cout << "=====================================================\n";
    ...
    for (const char* instance : {"input/TSPA.csv", "input/TSPB.csv"}) {
        ...
        // Baseline: Single pop with full 3 operators
        runConfig("Single Pop (20) + 3 Operators", [&](std::mt19937& rng) {
            return runFullAMSEA(n, selectCount, distance, costs, timeLimit, rng);
        });
        
        // Island configs with full 3 operators
        runConfig("2 Islands (10+10), migrate 100", [&](std::mt19937& rng) {
            return runFullAMSEA_Islands(n, selectCount, distance, costs, timeLimit, rng, 20, 2, 100);
        });
        
        runConfig("2 Islands (10+10), migrate 200", [&](std::mt19937& rng) {
            return runFullAMSEA_Islands(n, selectCount, distance, costs, timeLimit, rng, 20, 2, 200);
        });
        
        runConfig("4 Islands (5+5+5+5), migrate 50", [&](std::mt19937& rng) {
            return runFullAMSEA_Islands(n, selectCount, distance, costs, timeLimit, rng, 20, 4, 50);
        });
        
        runConfig("4 Islands (5+5+5+5), migrate 100", [&](std::mt19937& rng) {
            return runFullAMSEA_Islands(n, selectCount, distance, costs, timeLimit, rng, 20, 4, 100);
        });
    }
