// Full AMSEA with Optimized ERX
// Based on benchmark: ERX + Repair (0.016ms) is faster than CommonNodes + Repair (0.089ms)
...
// Optimized ERX with fixed-size adjacency (5x faster than original)
...
// AMSEA with optimized ERX as primary operator
struct Result {
    int bestObjective;
    long long generations;
};

Result runAMSEA_OptERX(int n, int selectCount,
    const std::vector<std::vector<int>> &distance,
    ...
    while (true) {
        ...
        // Use optimized ERX (fast and often produces full solution)
        std::vector<int> offspring = optimizedERX(pop[p1], pop[p2], n, selectCount, rng);
        
        // Repair only if needed (ERX usually produces full solution)
...
int main() {
    std::cout << "AMSEA with Optimized ERX (20 runs)\n";
    std::cout << "==================================\n\n";
    ...
    for (const auto &instance : instances) {
        ...
        for (int run = 0; run < numRuns; run++) {
            ...
            std::mt19937 rng(rd());
            auto result = runAMSEA_OptERX(n, selectCount, distance, costs, timeLimit, rng);
